...
/**
 * Texture de marque (vagues SVG) reprise d'aides-simplifiees.
 * Le tracé des vagues s'adapte par paliers à la hauteur du conteneur parent.
 * Dark mode mis de côté pour l'instant : seules les variantes claires sont utilisées.
 */
...
const wave = computed<WaveDefinition | null>(() => {
  ...
  // Plus grand palier inférieur à la hauteur courante du conteneur.
  return (
    [...waves]
...
.waves-container .waves-background,
.waves-container .waves-foreground {
  width: 100%;
}

.waves-container:not(.waves-container--xs) {
  display: flex;
  flex-direction: column;
}

.waves-container:not(.waves-container--xs) .waves-background,
.waves-container:not(.waves-container--xs) .waves-foreground {
  flex: 1;
}

.waves-container--xs .waves-background,
.waves-container--xs .waves-foreground {
  height: 25px;
}
